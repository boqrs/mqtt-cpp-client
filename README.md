...
|                                                                                      |
|   +---------------------+       +---------------------+       +---------------------+|
|   |   ProtocolParser    |       |   ProtocolParser    |       |   ProtocolParser    ||
|   |   - parseAndHandle  |       |   - parseAndHandle  |       |   - parseAndHandle  ||
...
|              |                             |                             |           |
|  +-----------+-----------+   +-----------+-----------+   +-----------+-----------+  |
|  |   LightService        |   |   StreamService      |   |   TempService        |  |
...
|   |   LightController   |       |   StreamController  |       |   TempController    ||
|   |   - turnOn          |       |   - startStream     |   |   - setTemperature     ||
|   |   - turnOff         |       |   - stopStream      |   |   - getTemperature     ||
|   +---------------------+       +---------------------+       +---------------------+|
|                                                                                      |
+--------------------------------------------------------------------------------------+    
```
...
└─────────────────────┘     └──────────┬──────────┘
    ...
    ┌──────────────────┴──────────────────┐                       ▼
    │ 是：日志报错，返回失败              │        ┌─────────────────────┐
    │ 否：指数退避重连，回到「阻塞等待连接状态」 │        │ 标记初始化完成，返回成功 │
    └─────────────────────────────────────┘        └─────────────────────┘
   ```
   2.2 自动重连流程
   ```
...
(3) 命令消息处理流程
![process cmd](./images/cmd_processor.png)
